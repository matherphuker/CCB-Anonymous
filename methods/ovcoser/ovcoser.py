import open_clip
import os
# from utils.loralib.utils import apply_lora, mark_only_lora_as_trainable, get_lora_parameters
# from ...lora_clip import clip
import torch
...
from open_clip.constants import OPENAI_DATASET_MEAN, OPENAI_DATASET_STD, IMAGENET_MEAN, IMAGENET_STD
from torch import nn
from .base_ops import LayerNorm2d, PixelNormalizer, rescale_2x, resize_to
...
os.environ['TORCH_HOME'] = './TORCHMODEL'
...
class ConvNeXtCLIP(nn.Module):
    def __init__(
        ...
        logger.info(f"Create the CLIP ({model_name + '-' + pretrained})")
    ...
    def get_text_embs_by_template(self, text_list):
        ...
        for text in text_list:
            ...
            text_tokens = self.text_tokenizer([template.format(text) for template in template_set]).to(self.device)
        ...
        text_embs = text_embs.mean(1) # Nc,768
        text_embs /= text_embs.norm(dim=-1, keepdim=True)
    ...
    @torch.no_grad()
    def get_image_emb(self, x, normalize=True):
        self.eval()
        emb = self.clip_model.encode_image(x)  # [B, D]
        if normalize:
...
class DinoV2Encoder(nn.Module):
    def __init__(self, model_name="dinov2_vitl14_reg"):
        ...
        self.model = torch.hub.load('TORCHMODEL/hub/facebookresearch_dinov2_main', model_name, trust_repo=True, source='local')
        ...
        layer_ids_dict  = {
            "stem": self.num_blocks // 12,       
            "res2": self.num_blocks // 6 + 1,     
            "res3": self.num_blocks // 2 - 1,      
            "res4": 3 * self.num_blocks // 4 - 1,  
            "res5": self.num_blocks - 1          
        }
        self.layer_ids = list(layer_ids_dict.values())
        self.out_channels = [192, 192, 384, 768, 1536]
        self.strides = [2, 4, 8, 16, 32]
        
        
        self.proj_layers = nn.ModuleList([
            nn.Sequential(
                nn.Conv2d(self.hidden_dim, ch, kernel_size=3, padding=1, bias=False),
                nn.BatchNorm2d(ch),
    ...
    def get_visual_feats(self, images):
        self.model.eval()
        images = self.normed(images)
        B, _, H, W = images.shape
        
        with torch.no_grad():
            feats = self.model.get_intermediate_layers(
                images, n=self.num_blocks, return_class_token=True
            ) 

        selected_feats = [feats[i][0] for i in self.layer_ids]  

        output_maps = []
        for i, feat in enumerate(selected_feats):
            patch_tokens = feat # (B, 32*32, 1024) for 448*448 input
            side_len = int(patch_tokens.shape[1] ** 0.5)  

            fmap = patch_tokens.permute(0, 2, 1).reshape(B, self.hidden_dim, side_len, side_len)


            target_H = H // self.strides[i]
            target_W = W // self.strides[i]

            fmap = F.interpolate(fmap, size=(target_H, target_W), mode='bilinear', align_corners=False)

            proj = self.proj_layers[i](fmap)  # [B, out_channels[i], H', W']
            output_maps.append(proj)
            

        final_feats = feats[-1]  #  [B, 1+N, C]
        # final_cls = final_feats[1]

        return output_maps, final_feats
    ...
    @staticmethod
    def normed(image):
        mean = torch.tensor(list(IMAGENET_MEAN), device=image.device).view(1, 3, 1, 1)
        std = torch.tensor(list(IMAGENET_STD), device=image.device).view(1, 3, 1, 1)
...
class OVCoser(nn.Module):
    def __init__(self, iterations=2, template_set="camoprompts"):
        ...
        self.fusion_convs = nn.ModuleList([
            nn.Conv2d(clip_channel + dino_channel, clip_channel, kernel_size=1)
            for clip_channel, dino_channel in zip([192, 192, 384, 768, 1536], [192, 192, 384, 768, 1536])
        ])
    ...
    def get_visual_feats(self, image):
        ...
        clip_image_feats = self.clip.get_visual_feats(image)
        dino_image_feats, dino_final_feats = self.dino.get_visual_feats(image)
        dino_cls_token = dino_final_feats[1]
        dino_final_feats = dino_final_feats[0]
        
        feats = []
        clip_feats = [
            rescale_2x(clip_image_feats["stem"]),
            clip_image_feats["res2"],
            clip_image_feats["res3"],
            clip_image_feats["res4"],
            clip_image_feats["res5"],
        ]

        for idx in range(5):
            clip_feat = clip_feats[idx]
            dino_feat = dino_image_feats[idx]

            fused = torch.cat([clip_feat, dino_feat], dim=1) 
            fused = self.fusion_convs[idx](fused) 
            feats.append(fused)
        
        image_stem = self.tra1(feats[0])
        image_res2 = self.tra2(feats[1])
        image_res3 = self.tra3(feats[2])
        image_res4 = self.tra4(feats[3])
        image_res5 = self.tra5(feats[4])

        image_deep = clip_image_feats["clip_vis_dense"]
        return image_stem, image_res2, image_res3, image_res4, image_res5, image_deep, dino_cls_token, dino_final_feats
    
    
    def no_map_classifier(self, logits, image_deep, normed_class_embs):
        image_embs = resize_to(image_deep, tgt_hw=logits.shape[-2:])

        image_embs = image_embs.mean((-1, -2))[..., None, None]  # (B, C, 1, 1)

        # B,C => B,D
        normed_image_embs = self.clip.visual_feats_to_embs(image_embs, normalize=True)
        

        class_logits = torch.einsum('bd,fnd->bfn', normed_image_embs, normed_class_embs)
    ...
    def map_classifier(self, logits, image_deep, normed_class_embs):
        ...
        # B,C => B,D
        normed_image_embs = self.clip.visual_feats_to_embs(image_embs, normalize=True)
        clip_foreground_embs = normed_image_embs.clone().detach()
        class_logits = normed_image_embs @ normed_class_embs.T  # （batchsize，embedding dimension）* (class numbers, embedding dimension)T = B,N
        class_logits = self.clip.clip_model.logit_scale.exp() * class_logits
        return class_logits, clip_foreground_embs
    

    def body(self, image, normed_class_embs):
        image_stem, image_res2, image_res3, image_res4, image_res5, image_deep, dino_cls_token, dino_final_feats = self.get_visual_feats(image)
        ...
        logits = {}
        logits[f"image_deep"] = image_deep
        logits[f"dino_cls_token"] = dino_cls_token
        logits[f"dino_final_feats"] = dino_final_feats
        cls_logits = None
        seg_logits = None
        for i in range(1, self.iterations + 1):
            ...
            cls_logits, logits["clip_foreground_embs"] = self.map_classifier(seg_logits, image_deep, normed_class_embs)
    ...
    def train_forward(self, data, gt_classes, class_names: list, **kwargs):
        ...
        return dict(
            vis={k: v if k == "edge" else v.sigmoid() for k, v in logits_dict.items() if not (k.startswith("cls") or k.startswith("dino") or k.startswith("clip") or k=="image_deep")},
            loss=sum(losses),
    ...
    def test_forward(self, data, gt_classes, class_names: list, **kwargs):
        ...
        dino_patch_tokens = logits_dict[f"dino_final_feats"]
        dino_cls_tokens = logits_dict[f"dino_cls_token"]
        clip_global_embs = self.clip.visual_feats_to_embs(logits_dict["image_deep"].mean((-1, -2))[..., None, None], normalize=True)
        ...
        return dict(
            ...
            clip_global_embs=clip_global_embs
        )
